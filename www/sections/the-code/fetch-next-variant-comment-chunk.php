...
$mysqlquery = "SELECT *, up_votes-down_votes as rating FROM variant_comments WHERE code_id='$code_id' AND parent='0' $sfw_sql $hide_shit_sql ORDER BY $orderby_sql LIMIT $comment_limit OFFSET $offset";
...
while ($row = $res->fetch_assoc()) {
  ...
  //get children
  if ($tandc_seen < 2) {
    $mysqlquery_child_replies = "SELECT *, up_votes-down_votes as rating FROM variant_comments WHERE code_id='$code_id' AND parent='$scenario_id' AND sfw < 2 ORDER BY post_time DESC, id ASC";
  } else {
    $mysqlquery_child_replies = "SELECT *, up_votes-down_votes as rating FROM variant_comments WHERE code_id='$code_id' AND parent='$scenario_id' ORDER BY post_time DESC, id ASC";
  }
