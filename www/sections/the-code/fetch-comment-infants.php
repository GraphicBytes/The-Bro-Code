...
$mysqlquery_infant_replies = "SELECT *, up_votes-down_votes as rating FROM thecode_examples WHERE code_id='$code_id' AND child='$scenario_id' $sfw_sql $hide_shit_sql ORDER BY post_time ASC, id ASC";


$mysqlquery_get_parent = "SELECT parent FROM thecode_examples WHERE code_id='$code_id' AND id='$scenario_id' ORDER BY post_time DESC, id ASC";
...
<div class="infant-<?php echo $scenario_id; ?>" style="display:none;">
  <div class="infant-container infant-container-<?php echo $scenario_id; ?>">
    ...
    while ($infant_row = $infant_res->fetch_assoc()) {
      ...
      if ($logged_in_id > 0) {
        $vote = 2;
        $mysqlquery2 = "SELECT * FROM thecode_example_ratings WHERE user_id='$logged_in_id' AND example_id='$infant_scenario_id' ORDER BY id DESC";
    ...
    ?>
      ...
      <div class="comment comment-box-<?php echo $infant_scenario_id; ?> infant border-top">
        ...
        <span class="comment-header">
          <span class="author-name"><img class="user-icon profile-icon" src="<?php echo $base_url ?>/styles/images/svgs/user-icon.svg?v=<?php echo $generator_v_num; ?>" alt="The Bro Code - <?php echo $infant_scenario_owner_name; ?>" /> 
                     <?php echo $infant_scenario_owner_name; ?></span>
        ...
        <span class="meta border-top">
          ...
          <span class="meta-right meta-replies-right code-meta-rating-<?php echo $infant_scenario_id; ?>">

            <?php if ($logged_in_id > 0 && $infant_scenario_owner_id != $logged_in_id) { ?>
            ...
            <?php if ($infant_scenario_owner_id == $logged_in_id) { ?>
  ...
  <?php if ($logged_in_id > 0) : ?>
    ...
    <div class="comment infant reply-form-container-<?php echo $scenario_id; ?>">
      ...
      <form method="post" class="reply-form reply-form-<?php echo $scenario_id; ?>" action="<?php echo $base_url; ?>/replyto-code-comment/<?php echo $code_id; ?>/<?php echo $parent_id; ?>/<?php echo $scenario_id; ?>/" enctype="multipart/form-data">
