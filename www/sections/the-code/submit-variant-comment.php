...
if ($character_count > 1000) {
  ...
  // $indexstatus=0;
  // $res = $db->sql( "SELECT * FROM underground_codesindex WHERE id=? ORDER BY id DESC LIMIT 1", 'i' , $code_id );
  ...
  $mysqlquery = "INSERT INTO variant_comments SET owner_id=?, code_id=?, parent=?, content=?, post_time=?, last_update=?";
  $db->sql($mysqlquery, 'iiisii', $logged_in_id, $code_id, $parent_id, $comment, $post_time, $post_time);
  ...
  ob_start();
  $mysqlquery = "SELECT *, up_votes-down_votes as rating FROM variant_comments WHERE code_id='$code_id' AND owner_id='$logged_in_id' ORDER BY post_time DESC, id ASC LIMIT 1";
