...
class Node_Container_Document extends Node_Container
{
	...
	public static function default_bbcodes()
	{
		return array(
			...
			new BBCode('icq', '<a href="https://www.icq.com/people/about_me.php?uin=%content%">
				<img  src="https://status.icq.com/online.gif?icq=%content%&amp;img=5"> %content%</a>',
			...
			{
				if(substr($content, 0, 24) === 'https://player.vimeo.com/')
					$uri = $content;
				else if(substr($content, 0, 17) === 'https://vimeo.com/'
					|| substr($content, 0, 21) === 'https://www.vimeo.com/'
					&& preg_match("/http:\/\/(?:www\.)?vimeo\.com\/([0-9]{4,10})/", $content, $matches))
				{
					preg_match("/http:\/\/(?:www\.)?vimeo\.com\/([0-9]{4,10})/", $content, $matches);
					$uri = 'https://player.vimeo.com/video/' . $matches[1] . '?title=0&amp;byline=0&amp;portrait=0';
				}
				else
					$uri = 'https://player.vimeo.com/video/' . $content . '?title=0&amp;byline=0&amp;portrait=0';
			...
			new BBCode('pastebin', function($content, $attribs)
			{
				...
				{
					preg_match("#https://pastebin.com/([a-zA-Z0-9]+)#", $content, $matches);
					$content = '';
				...
				return '<script src="https://pastebin.com/embed_js.php?i=' . $content . '"></script>';
			}, BBCode::INLINE_TAG, false, array(), array('text_node'), BBCode::AUTO_DETECT_EXCLUDE_ALL),
			...
			new BBCode('twitter', '<a href="https://twitter.com/%content%"
				...
				BBCode::INLINE_TAG, false, array(), array('text_node'), BBCode::AUTO_DETECT_EXCLUDE_ALL),
			new BBCode('tweets', "<script src=\"https://widgets.twimg.com/j/2/widget.js\"></script>
				<script>
				new TWTR.Widget({
					version: 2,
					type: 'profile',
			...
			new BBCode('googlemaps', '<iframe src="https://maps.google.com/maps?q=%content%&amp;output=embed"
				scrolling="no" width="100%" height="350" frameborder="0"></iframe>',
				BBCode::BLOCK_TAG, false, array(), array('text_node'), BBCode::AUTO_DETECT_EXCLUDE_ALL),

			new BBCode('pdf', '<iframe src="https://docs.google.com/gview?url=%content%&amp;embedded=true"
			...
			new BBCode('scribd', function($content, $attribs)
			{
				...
				return '<iframe src="https://www.scribd.com/embeds/' . $attribs['id'] . '/content?start_page=1&view_mode=list&access_key=' . $attribs['key'] . '"
			...
			new BBCode('size', function($content, $attribs)
			{
				...
				*/
				...
				if ($attribs['default'] == 'very-big') {
					$spanheader = '<h2>' . $content . '</h2>';
				} else if ($attribs['default'] == 'big'){
					$spanheader = '<h3>' . $content . '</h3>';
				} else if ($attribs['default'] == 'normal'){
					$spanheader = '<h4>' . $content . '</h4>';
				} else if ($attribs['default'] == 'small'){
					$spanheader = '<h5>' . $content . '</h5>';
				} else if ($attribs['default'] == 'very-small'){
					$spanheader = '<h6>' . $content . '</h6>';
				} else {
					$spanheader = '<h4>' . $content . '</h4>';
				}
			...
			new BBCode('img', function($content, $attribs, $node)
			{
				...
				// add https:// to www starting urls
			...
			new BBCode('url', function($content, $attribs, $node)
			{
				...
				if(strpos($attribs['default'], 'www') === 0)
					$attribs['default'] = 'https://' . $attribs['default'];
				// add the base url to any urls not starting with http or ftp as they must be relative
